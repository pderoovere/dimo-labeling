...
class PoseCalculator:
    ...
    def calc_pose(self, points, pixels, camera_matrix):
        ...
        _, rvecs, tvecs, _ = cv2.solvePnPRansac(points, pixels, camera_matrix, None, iterationsCount=100_000)
        T = self.T_from_rvec_tvec(rvecs, tvecs)
        T_ref = self.t_from_xyzwpr(w=180.0)
        T1 = np.linalg.pinv(T_ref) @ T
        T2 = T_ref @ T
        return T2
    ...
    def T_from_rvec_tvec(self, rvec, tvec):
        T = np.eye(4, 4)
        R = np.zeros((3, 3))
        assert rvec.shape == (3, 3) or rvec.shape == (3, 1), f"Illegal rvec shape {rvec}"
        if rvec.shape == (3, 3):
            R = rvec
        else:
