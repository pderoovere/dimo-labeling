...
class DatasetLoader:
    ...
    def load(self, path, args):
        ...
        parts = dict((part_id, self.load_part(path, parts_path, part_id)) for part_id in part_ids)
    ...
    def load_part(self, main_path, path, part_id):
        ...
        result = {
            'id': part_id, 'cadPath': str(f'/api/cdn/{cad_path.relative_to(main_path)}'), 'texturePath': ''
        }
        texture_path = path / f'obj_{part_id:06d}.png'
        if texture_path.exists():
            result['texturePath'] = str(f'/api/cdn/{texture_path.relative_to(main_path)}')
        return result
    ...
    def load_scene(self, parts, main_path, path):
        ...
        with open(path / 'scene_gt.json') as f_gt, open(path / 'scene_camera.json') as f_cam:
            ...
            images = [self.load_image(main_path, path, image_id, images_cam[image_id])
                      for image_id in images_gt]
            positioned_parts = self.load_positioned_parts(path, parts, images_gt[images[0]['id']], images)
            return {
                'id': scene_id,
                'images': images,
    ...
    def convert_positioned_parts_from_camera_to_world(self, positioned_parts, camera_2world):
        for positioned_part in positioned_parts:
            pose = self.list_to_pose(camera_2world) @ self.list_to_pose(positioned_part['pose'])
