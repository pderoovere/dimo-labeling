...
class DatasetSaver:
    ...
    def save_scene_data(self, path, scene_data):
        ...
        converted_data = dict(
            [
                self.convert_image_data(image_data, positioned_parts)
                for image_data in scene_data["images"]
            ]
        )
        ...
        with open(path / _id / "scene_gt_world.json", "w") as f:
            gt_world = [
                self.convert_positioned_part_world(positioned_part)
    ...
    def convert_image_data(self, image_data, positioned_parts):
        ...
        converted_parts = [
            self.convert_positioned_part(positioned_part, camera_pose)
            for positioned_part in positioned_parts
        ]
    ...
    def convert_positioned_part(self, positioned_part, camera_pose):
        ...
        part_2cam = np.linalg.inv(camera_pose) @ part_2world
    ...
    def convert_data(self, pose, part_id, key_R, key_t):
        ...
        return {key_R: pose_R, key_t: pose_t, "obj_id": part_id}
